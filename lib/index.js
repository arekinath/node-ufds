...
var InternalError = restify_errors.InternalError;
var InvalidArgumentError = restify_errors.InvalidArgumentError;
var InvalidCredentialsError = restify_errors.InvalidCredentialsError;
var MissingParameterError = restify_errors.MissingParameterError;
var NotAuthorizedError = restify_errors.NotAuthorizedError;
var ResourceNotFoundError = restify_errors.ResourceNotFoundError;
var HttpError = restify_errors.HttpError;
...
function filterEqual(attrname, valstr) {
...
function filterObjectClass(ocname, andFilter) {
    return (new ldap.filters.AndFilter({
        filters: [
            filterEqual('objectclass', ocname),
            andFilter
        ]
    }));
}
...
/*
 * Many of the functions exposed to consumers of the library accept an optional
 * "account" argument.  This argument is positional, so its absence is often
 * detected by attempting to locate the subsequent callback (or other)
 * argument.  Due to accidents of history, the user may also have supplied one
 * of the many possible falsey values.  This routine attempts to normalise the
 * most likely falsey arguments the user might pass, reducing to either a
 * string or undefined -- enabling us to use the "optionalString" check from
 * "assert-plus" in stricter functions.
 *
 * In the next major release, this should all be replaced with stricter type
 * checking and option objects instead of variadic functions.
 */
function workAroundUnderspecification(astring, name) {
    if (astring === undefined || astring === null || astring === false) {
        return (undefined);
    }
    assert.string(astring, name);
    return (astring);
}
 ...
 * contains no properties other than those expected by the software.
 *
 * The "expected" parameter is a map from expected property name to the type
...
function assertStrictOptions(funcname, opts, expected) {
    ...
    for (var k in opts) {
        ...
        var e = expected[k];
        if (!e) {
            throw (new Error(funcname + ': unexpected option: ' + k));
        }

        assert[e](opts[k], 'opts.' + k);
    }
}
...
UFDS.prototype.authenticate = function authenticate(login, pwd, account, cb) {
    ...
    account = workAroundUnderspecification(account, 'account');
...
UFDS.prototype.addUser = function addUser(user, cb) {
    ...
    var dn;
    if (user.account) {
        dn = sprintf(SUBUSER_FMT, user.uuid, user.account);
    } else {
        dn = sprintf(USER_FMT, user.uuid);
    }

    var self = this;

    self.add(dn, user, function (add_err) {
        if (add_err) {
            cb(add_err);
            return;
        }
...
/*
 * Internal function for the common elements of fetching a user from UFDS.
 *
 * The "filter" is a Filter object from the ldapjs Filter API.  The search
 * filter sent to the server will be the conjunction of the provided filter
 * and a filter to select only user objects; i.e. (objectclass=sdcperson).
 *
 * The "msg" is a string to use as the message for an exception if the
 * account is not found.
 *
 * The "account" is an optional UUID parameter.  It must be provided if this
 * is a search for sub-users under an account.  Note that the "filter"
 * should be set appropriately if fetching an account by login name.
 *
 * The function argument "cb" will be called with the results.
 *
 * The "noCache" option is an optional boolean to force a full directory
 * lookup.
 */
UFDS.prototype._getUser = function _getUser(filter, msg, account, cb, noCache) {
    assert.ok(ldap.filters.isFilter(filter), 'filter');
    assert.string(msg, 'msg');
    assert.optionalString(account, 'account');
    assert.func(cb, 'callback');
    assert.optionalBool(noCache, 'noCache');
    ...
    var opts = {
        scope: 'one',
        filter: filterObjectClass('sdcperson', filter)
    };
...
/*
 * Fetch a user from UFDS.  The user may be selected by login name or by
 * UUID.
 *
 * The "opts" object must contain a valid "searchType" and "value".  Both
 * "uuid" and "login" are valid search types; "value" will be interpreted
 * accordingly.  The caller may also provide "account"; sub-users of this
 * account will then be searched instead of top-level accounts.
 *
 * The object may contain the "noCache" property, a boolean, to force an
 * uncached directory lookup.
 *
 * Providing invalid values for options, or options beyond those documented
 * to work, will cause the function to throw.
 */
UFDS.prototype.getUserEx = function getUserEx(opts, cb) {
    assert.func(cb, 'cb');
    assertStrictOptions('getUserEx', opts, {
        'account': 'optionalString',
        'searchType': 'string',
        'value': 'string',
        'noCache': 'optionalBool'
    });
    ...
    case 'uuid':
        /*
         * Note that the UUID filter is the same whether or not we are looking
         * for a top-level account or a sub-user.  In either case, the base DN
         * of the search is set appropriately by _getUser().
         */
        filter = filterEqual('uuid', opts.value);
        break;

    case 'login':
        if (opts.account) {
            filter = filterEqual('login', opts.account + '/' + opts.value);
        } else {
            filter = filterEqual('login', opts.value);
        }
        break;

    default:
        throw (new Error('invalid "searchType": ' + opts.searchType));
    }

    var msg = sprintf('user with %s "%s" does not exist', opts.searchType,
        opts.value);

    this._getUser(filter, msg, opts.account, cb, opts.noCache);
};
...
UFDS.prototype.getUser = function getUser(login, account, cb, noCache) {
    ...
    var loginStr = account ? sprintf('%s/%s', account, login) : login;
    var filter = new ldap.filters.OrFilter({
        filters: [
            filterEqual('login', loginStr),
            filterEqual('uuid', login)
        ]
    });
...
function getUserByEmail(email, account, cb, noCache) {
    ...
    var filter = filterEqual('email', email);
...
UFDS.prototype.updateUser = function updateUser(user, changes, account, cb) {
    ...
    if (typeof (user) !== 'string') {
        assert.object(user, 'user');
        assert.optionalUuid(user.account, 'user.account');
        if (user.account) {
            if (account) {
                /*
                 * If the caller is updating a sub-user of a top-level
                 * account, they will have provided the UUID of that account
...
UFDS.prototype.addKey = function addKey(user, key, account, cb) {
    ...
    function _addKey(init_err, user) {
        ...
        var fingerprint;
        try {
            var key = sshpk.parseKey(key.openssh, 'ssh');
            fingerprint = key.fingerprint('md5').toString('hex');
        } catch (e) {
